...
//    ConcurrentHashMap<String, SeqCache> seqBufferMap = new ConcurrentHashMap<>();
...
//            SeqCache seqBuffer = seqBufferMap.get(seqName);
...
//        seqBufferMap.putIfAbsent(seqName, new SeqCache());
//        OperatingSeqSegment segmentFromDb = getSegmentFromDb(seqName, seqInfoMap.get(seqName).getServerCacheSize());
...
//                for (Map.Entry<String, SeqCache> entry : seqBufferMap.entrySet()) {
//                    if (!fetchingSequenceSet.contains(seqInfoMap.get(entry.getKey())) && entry.getValue().getTotal() < (long) seqInfoMap.get(entry.getKey()).getServerCacheSize() / 2) {//只要用掉了缓存，就拉取到满
//                        //这个地方提交任务是为了防止死循环，cpu占用高，实际上可能有多个线程同时执行一个序列的任务
//                        fetchingSequenceSet.add(seqInfoMap.get(entry.getKey()));
//                        fetchService.execute(() -> fetchTask.accept(entry.getKey(), seqInfoMap.get(entry.getKey()).getServerCacheSize()));
//                    }
//                }
...
//        //对于普通唯一的，所有节点都能服务
//        int cacheSize = seqInfoMap.get(seqName).getServerCacheSize();
